...
async def handle_clerk_event(
    ...
    *,
    ...
    if not is_first_time:
        log_business_event("webhooks.deduplicated", 1, {"event_type": event_type})
        return "already_processed"

    if event_type == "user.created":
        await handle_user_created(db, data)
        log_business_event("webhooks.user_created", 1)
    elif event_type == "user.updated":
        await handle_user_updated(db, data)
        log_business_event("webhooks.user_updated", 1)
    elif event_type == "user.deleted":
        await handle_user_deleted(db, data)
        log_business_event("webhooks.user_deleted", 1)
    else:
        log_business_event("webhooks.ignored", 1, {"event_type": event_type})
        return "processed"

    log_business_event("webhooks.processed", 1, {"event_type": event_type})
    return "processed"
